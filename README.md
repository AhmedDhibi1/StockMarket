...
mongodb	Spring Data MongoDB with optimistic locking and AOP retry
Market Data	finhub	Real-time quotes from Finnhub
alphaVantage	Real-time quotes from Alpha Vantage
mockfinhub	Random-but-reasonable prices — no API key needed (used by tests)
Valid Combinations
jpa,finhub
jpa,alphaVantage
jpa,mockfinhub
...
Example Workflow
POST /api/portfolios — Create a portfolio for "Alice"
POST /api/portfolios/{id}/deposits — Deposit $50,000
POST /api/portfolios/{id}/purchases — Buy 10 shares of AAPL
GET /api/portfolios/{id}/holdings — Check unrealized gains
POST /api/portfolios/{id}/sales — Sell 5 shares of AAPL (FIFO)
GET /api/portfolios/{id}/transactions — View transaction history
Running with Docker
...
Error Handling
The API uses RFC 7807 Problem Details for error responses. Global exception handling (ExceptionHandlingAdvice) maps domain and application exceptions to appropriate HTTP status codes:
Feuilles de calcul
...
Checkout with full fetch depth (for SonarCloud analysis)
Setup JDK 21 (Temurin) with Maven caching
Docker diagnostics (version info for debugging Testcontainers issues)
Build & Test: ./mvnw -B -U -ntp clean verify
Failure diagnostics: Prints Surefire and Failsafe reports on failure
Artifacts: Uploads test results and JaCoCo coverage reports
SonarCloud: Quality gate analysis (requires manual setup per CI_SETUP.md)
Quality Gates
All tests must pass
